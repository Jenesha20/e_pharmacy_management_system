            ...
            <a href="../../shop/product/product.html?id=${product.id}">
                <div class="bg-[#A1E970] bg-opacity-50 p-4 flex justify-center items-center">
                    <img src="${product.image_url || 'https://via.placeholder.com/150'}" alt="${product.name}" 
                         class="h-32 object-contain" onerror="this.src='https://via.placeholder.com/150'"/>
                </div>
            </a>
            <div class="p-4">
...
async function addToCart(productId) {
    ...
    // Case 1: User is logged in → try to save to backend
    const currentUser = getCurrentUser();
    if (currentUser) {
      const userId = currentUser.id;
      ...
      try {
        ...
        // If all backend attempts fail, fall back to localStorage
        console.warn("All backend attempts failed, using localStorage fallback");
        addToLocalStorageCart(productId, now);
  ...
  // Helper function to add to localStorage cart
  function addToLocalStorageCart(productId, timestamp) {
    let guestCart = JSON.parse(localStorage.getItem("guestCart")) || [];
  ...
  function getProductImage(productId) {
    const product = products.find(p => p.id == productId || p.product_id == productId);
    return product ? product.image_url : 'https://via.placeholder.com/150';
  }
