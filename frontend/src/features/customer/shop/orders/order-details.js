...
//   // FIX: Better handling of items structure
...
function displayOrderItems() {
  ...
  itemsContainer.innerHTML = currentOrder.items.map(item => {
    ...
    const subtotal = item.subtotal || (item.unit_price || item.price || 0) * (item.quantity || 1);
...
function displayOrderInformation() {
  ...
  const subtotal = currentOrder.subtotal || (currentOrder.items ? 
    currentOrder.items.reduce((sum, item) => sum + (item.subtotal || (item.unit_price || item.price || 0) * (item.quantity || 1)), 0) : 
