...
//       const res = await fetch(`http://localhost:3000/prescriptions?user_id=${userId}`);
//       if (!res.ok) throw new Error("Failed to fetch prescriptions");
//       prescriptions = await res.json();
//       renderPrescriptionList();
//       updateUploadState();
...
// // Select prescription
// async function selectPrescription(productId, label, prescriptionId) {
//   document.getElementById(`selected-${productId}`).innerText = label;
//   document.getElementById(`dropdown-${productId}`).classList.add("hidden");
...
//       if (itemToUpdate && itemToUpdate.cartItemId) {
//         // Update the cart item with prescriptionId
//         await fetch(`http://localhost:3000/cart/${itemToUpdate.cartItemId}`, {
//           method: "PATCH",
//           headers: { "Content-Type": "application/json" },
//           body: JSON.stringify({ prescriptionId })
...
//         // Create a prescription record linking the medicine to the prescription
...
//         // Save the prescription-medicine relationship
//         await fetch("http://localhost:3000/prescription_medicine", {
//           method: "POST",
//           headers: { "Content-Type": "application/json" },
//           body: JSON.stringify(prescriptionRecord)
//         });
//       }
//     } catch (err) {
//       console.error("Error updating prescription selection:", err);
//     }
//   } else {
//     // For guest users, save to local storage
//     saveGuestData();
...
//           // Find if this prescription was linked to any cart items
//           const cartItemsWithThisPrescription = guestCart.filter(item => 
//             item.prescriptionId == prescription.id
//           );
          
//           // Create prescription-medicine relationships
//           for (const cartItem of cartItemsWithThisPrescription) {
//             const prescriptionRecord = {
//               prescription_id: savedPrescription.id,
//               medicine_id: cartItem.product_id,
//               user_id: userId
//             };
