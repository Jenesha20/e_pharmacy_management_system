...
let prescriptions = []; // {id, fileName, data, userId, medicineIds}
let cartItems = []; // fetched cart items
let productsRequiringPrescription = 0;
let isLoggedIn = false;
...
// Fetch prescriptions - MODIFIED to include user filtering
async function fetchPrescriptions() {
  if (isLoggedIn) {
    try {
      const currentUser = JSON.parse(localStorage.getItem("currentUser"));
      ...
      const res = await fetch(`http://localhost:3000/prescriptions?userId=${userId}`);
      if (!res.ok) throw new Error("Failed to fetch prescriptions");
      prescriptions = await res.json();
      renderPrescriptionList();
      updateUploadState();
...
// Handle prescription upload - MODIFIED to support multiple medicine IDs
function handlePrescriptionUpload(event) {
  const file = event.target.files[0];
  if (!file) return;
  ...
  reader.onload = async function() {
    ...
    // Get the medicine IDs from the upload context
    const medicineIds = getSelectedMedicineIdsForUpload();
    
    const prescription = { 
      id: Date.now(), 
      fileName: file.name, 
      data: base64,
      medicineIds: medicineIds // Array of medicine IDs
    };

    if (isLoggedIn) {
      try {
        const currentUser = JSON.parse(localStorage.getItem("currentUser"));
        prescription.userId = currentUser.id; // Add user ID to prescription
...
// NEW FUNCTION: Get selected medicine IDs for upload (multiple selection)
...
// Function to transfer guest data to user account after login/registration - MODIFIED
async function transferGuestDataToUser(userId) {
  // Get guest data from local storage
  const guestCart = JSON.parse(localStorage.getItem('guestCart') || '[]');
  const guestPrescriptions = JSON.parse(localStorage.getItem('guestPrescriptions') || '[]');
  ...
  if (guestCart.length > 0 || guestPrescriptions.length > 0) {
    try {
      // Transfer prescriptions first with userId
      for (const prescription of guestPrescriptions) {
        await fetch("http://localhost:3000/prescriptions", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      // Transfer cart items with user_id
      for (const item of guestCart) {
        await fetch("http://localhost:3000/cart", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
